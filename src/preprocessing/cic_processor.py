...
def process_cic_dynamic():
    """Extracts PyTorch Geometric graph embeddings, flattens them, and merges with CSV metadata."""
    ...
    for index, row in df_meta.iterrows():
        ...
        if file_hash in pt_dict:
            try:
# FIX 1: Set weights_only=False because we trust this specific dataset
                # This allows PyTorch to load the complex torch_geometric Data object
                tensor = torch.load(pt_dict[file_hash], weights_only=False, map_location=torch.device('cpu'))
                
                # FIX 2: Handle the PyTorch Geometric Graph Object
                # Graph objects store their node features in the '.x' attribute
                ...
                # Mean pooling to flatten the graph into a 1D vector for XGBoost
                if arr.ndim > 1:
