...
# 🧠 THE QUAD-CORE JURY WEIGHTS
EXPERT_AUTHORITY = {
    'Quo_Vadis': 3.0,  # Dynamic Behavioral DNA (Highest weight)
    'EMBER': 2.5,      # Enterprise Headers
    'BODMAS': 1.5,     # Malware Families
    'Kaggle': 0.8      # Basic File Structure
}
...
class IntelliGuardEnsemble:
    def __init__(self):
        logger.info("🧠 Waking up IntelliGuard AI Ensemble (Quad-Core Edition)...")
        self.models = {}
        self._load_models()

    def _load_models(self):
        # We check both paths in case the dynamic model saved to the root 'models' folder
        model_paths = {
            ...
            'Quo_Vadis': Path("models/expert_quo_vadis_dynamic.json") 
        }
        
        # Fallback for Quo Vadis if moved to outputs/
        if not model_paths['Quo_Vadis'].exists():
            model_paths['Quo_Vadis'] = Path("outputs/models/expert_quo_vadis_dynamic.json")
        ...
        for name, path in model_paths.items():
            if path.exists():
                ...
                logger.info(f"✅ {name} Expert loaded successfully. (Authority Weight: {EXPERT_AUTHORITY.get(name, 1.0)}x)")
            else:
    ...
    def _proxy_dynamic_features(self, file_path, raw_features):
        """
        Since we don't have a live sandbox, this performs a deep binary 
        string scan to proxy the dynamic behavioral features for the Quo Vadis model.
        """
        try:
            ...
            inj_apis = [b'virtualallocex', b'writeprocessmemory', b'createremotethread', b'ntmapviewofsection']
            evas_apis = [b'isdebuggerpresent', b'checkremotedebuggerpresent', b'sleep', b'ntdelayexecution']
            rans_apis = [b'cryptencrypt', b'vssadmin', b'bcdedit']

            raw_features['has_injection'] = int(any(api in content for api in inj_apis))
            raw_features['has_evasion'] = int(any(api in content for api in evas_apis))
            raw_features['has_crypto_ransom'] = int(any(api in content for api in rans_apis))
            
            # Approximations for dynamic counts based on binary size/density
            raw_features['num_api_calls'] = content.count(b'import') * 5 
            raw_features['num_dynamic_segments'] = content.count(b'.text') 
            
        except Exception as e:
            logger.warning(f"⚠️ Failed to extract dynamic proxies: {e}")
    ...
    def scan_file(self, file_path):
        ...
        # PROXY BRIDGE: Inject dynamic features into the dataframe
        raw_features = self._proxy_dynamic_features(file_path, raw_features)
        ...
        for name, model in self.models.items():
            ...
            logger.info(f"[{name}] {status} (Prob: {prob:.4f}, Cov: {match_ratio*100:.1f}%, Weight: {weight:.2f})")
